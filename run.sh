...
for url in $SERVER/desktop/urls/*.txt ; do
  ...
  for browser in "${BROWSERS[@]}"
    do
      ...
      docker run $DOCKER_SETUP $DOCKER_CONTAINER $NAMESPACE $CONFIG/desktop.json -b $browser $url --plugins.add ./node_modules/@sitespeed.io/plugin-lighthouse/ REPLAY=true -e LATENCY=100       
